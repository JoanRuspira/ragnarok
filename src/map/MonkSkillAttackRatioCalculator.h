...
{
	public:
		static int calculate_skill_atk_ratio(struct block_list* src, struct block_list *target, int base_lv, int skill_id, int skill_lv, struct status_data* sstatus, bool revealed_hidden_enemy, bool is_using_knuckle);
	...
	private:
		...
		static int calculate_chain_combo_atk_ratio(int skill_lv, bool is_using_knuckle);
};
