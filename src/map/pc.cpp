...
int pc_skillheal_bonus(struct map_session_data *sd, uint16 skill_id) {
	...
	if( bonus ) {
		switch( skill_id ) {
			case CR_HEAL:           if( !(battle_config.skill_add_heal_rate&1) ) bonus = 0; break;
...
int pc_itemheal(struct map_session_data *sd, t_itemid itemid, int hp, int sp)
{
	...
	if (sp) {
		bonus = 100 + (sd->battle_status.int_ << 1) + (pc_checkskill(sd, MG_SRECOVERY)*2) * 10 + pc_checkskill(sd, AM_LEARNINGPOTION) * 5;
		// A potion produced by an Alchemist in the Fame Top 10 gets +50% effect [DracoRPG]
		if (potion_flag == 2)
			bonus += bonus * 50 / 100;
