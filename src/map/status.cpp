...
void initChangeTables(void)
{
	...
	add_sc( MG_SAFETYWALL		, SC_PNEUMA		);
	...
	// 
	( AL_CRUCIS		, SC_SIGNUMCRUCIS	, EFST_CRUCIS, SCB_DEF );
	...
	set_sc( HM_LANDBOOST		, SC_LANDBOOST		, EFST_LANDBOOST		, SCB_NONE);
	set_sc( HM_FIREBOOST		, SC_FIREBOOST		, EFST_FIREBOOST		, SCB_NONE);
	...
	set_sc( AS_CLOAKING		, SC_CLOAKING		, EFST_CLOAKING		, SCB_SPEED );
	...
	set_sc( AM_HATCH_HOMUNCULUS			, SC_BIOETHICS		, EFST_BIOETHICS, SCB_NONE );
	...
	set_sc( MO_KNUCKLEQUICKEN			, SC_KNUCKLEQUICKEN		, EFST_KNUCKLEQUICKEN, SCB_ASPD|SCB_WATK );
	...
	set_sc( MO_BODYRELOCATION	, SC_DEFENSIVESTANCE	, EFST_DEFENSIVESTANCE		, SCB_DEF|SCB_MDEF );
	...
	set_sc( SC_INVISIBILITY		, SC__INVISIBILITY	, EFST_INVISIBILITY	, SCB_NONE );
	...
	set_sc( SC_GROOMY		, SC__GROOMY		, EFST_GROOMY		, SCB_ASPD );
	set_sc( SC_IGNORANCE		, SC__IGNORANCE		, EFST_IGNORANCE		, SCB_DEF|SCB_MDEF );
...
static int status_get_spbonus(struct block_list *bl, enum e_status_bonus type) {
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (bl->type == BL_PC) {
			...
			if ((skill_lv = (pc_checkskill(sd,WM_LESSON)*2)) > 0)
				bonus += 30 * skill_lv;
			if (pc_checkskill(sd,SU_SPRITEMABLE) > 0)
				bonus += 100;
			if (pc_checkskill(sd, SU_POWEROFSEA) > 0) {
		...
		if (bl->type == BL_PC) {
			...
			if((i = (pc_checkskill(sd,HP_MEDITATIO)*2)) > 0)
				bonus += i;
			if((i = pc_checkskill(sd,HW_SOULDRAIN)) > 0)
				bonus += 2 * i;
#ifdef RENEWAL
			if ((i = (pc_checkskill(sd, (sd->status.sex ? BA_MUSICALLESSON : DC_DANCINGLESSON))*2)) > 0)
				bonus += i;
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_DEFENSIVESTANCE:
			if (sd)
				val1 = pc_checkskill(sd, MO_BODYRELOCATION) * 4; // DEF/MDEF Increase
			break;
