...
void initChangeTables(void)
{
	...
	add_sc( SK_SM_BASH			, SC_STUN		);
	set_sc( SK_SM_PROVOKE		, SC_PROVOKE		, EFST_PROVOKE		, SCB_DEF|SCB_DEF2|SCB_BATK|SCB_WATK );
	set_sc( SK_AL_SIGNUMCRUCIS		, SC_CRUCIS_PLAYER	, EFST_CRUCIS_PLAYER	, SCB_MATK|SCB_MDEF	);
	set_sc( SK_SM_MAGNUM		, SC_WATK_ELEMENT	, EFST_WATK_ELEMENT	, SCB_NONE	);
	set_sc( SK_MC_COINFLIP		, SC_COIN_DEX	, EFST_COIN_DEX	, SCB_DEX	);
	set_sc( SK_MC_COINFLIP		, SC_COIN_LUK	, EFST_COIN_LUK	, SCB_LUK	);
	set_sc( SK_SO_ASTRALSTRIKE		, SC_ASTRALSTRIKE_DEBUFF	, EFST_ASTRALSTRIKE_DEBUFF	, SCB_NONE	);
	set_sc( SK_BS_ENDURE		, SC_ENDURE		, EFST_ENDURE		, SCB_MDEF|SCB_DSPD|SCB_DEF );
	add_sc( SK_MG_FROSTDIVER		, SC_FREEZE		);
	add_sc( SK_MG_STONECURSE		, SC_STONE		);
	add_sc( SK_AL_RUWACH		, SC_RUWACH		);
	set_sc( SK_AL_INCREASEAGI		, SC_INCREASEAGI	, EFST_INC_AGI, SCB_AGI|SCB_SPEED|SCB_ASPD );
	set_sc( SK_MO_DECAGI		, SC_DECREASEAGI	, EFST_DEC_AGI, SCB_AGI|SCB_SPEED );

	set_sc( SK_AL_SIGNUMCRUCIS		, SC_SIGNUMCRUCIS	, EFST_CRUCIS, SCB_DEF );
	set_sc( SK_AL_ANGELUS		, SC_ANGELUS		, EFST_ANGELUS		, SCB_DEF2|SCB_MAXHP );

	set_sc( SK_AL_BLESSING		, SC_BLESSING		, EFST_BLESSING		, SCB_STR|SCB_INT|SCB_DEX);

	set_sc( SK_AM_PETROLOGY		, SC_LANDBOOST		, EFST_LANDBOOST		, SCB_NONE);
	set_sc( SK_AM_PYROTECHNIA		, SC_FIREBOOST		, EFST_FIREBOOST		, SCB_NONE);
	
	set_sc( SK_AC_IMPROVECONCENTRATION	, SC_CONCENTRATE	, EFST_CONCENTRATION, SCB_AGI|SCB_DEX );
	set_sc( SK_RA_SPIRITANIMAL	, SC_SPIRITANIMAL	, EFST_SPIRITANIMAL, SCB_INT|SCB_MATK );
	set_sc( SK_TF_HIDING		, SC_HIDING		, EFST_HIDING		, SCB_SPEED );
	add_sc( SK_TF_POISONSLASH		, SC_POISON		);
	set_sc( SK_KN_COUNTERATTACK		, SC_AUTOCOUNTER	, EFST_AUTOCOUNTER	, SCB_NONE );
	set_sc( SK_PR_IMPOSITIO		, SC_IMPOSITIO		, EFST_IMPOSITIO	, SCB_WATK);
	set_sc(SK_MS_ARMORREINFORCEMENT, SC_REINFORCEMENT, EFST_REINFORCEMENT, SCB_DEF);

	set_sc( SK_PR_SUFFRAGIUM		, SC_SUFFRAGIUM		, EFST_SUFFRAGIUM		, SCB_NONE );
	set_sc( SK_PR_ASPERSIO		, SC_ASPERSIO		, EFST_ASPERSIO		, SCB_ATK_ELE );
	set_sc( SK_BI_BENEDICTIO		, SC_BENEDICTIO		, EFST_BENEDICTIO		, SCB_DEF_ELE|SCB_WATK|SCB_MATK|SCB_AGI|SCB_VIT|SCB_LUK);
	set_sc( SK_PR_KYRIE		, SC_KYRIE		, EFST_KYRIE		, SCB_NONE );
	set_sc( SK_PR_MAGNIFICAT		, SC_MAGNIFICAT		, EFST_MAGNIFICAT		, SCB_REGEN );
	add_sc( SK_SA_SILENCE		, SC_SILENCE		);
	set_sc( SK_MG_QUAGMIRE		, SC_QUAGMIRE		, EFST_QUAGMIRE		, SCB_AGI|SCB_DEX|SCB_ASPD|SCB_SPEED );
	set_sc( SK_BS_WEAPONRYEXPERTISE		, SC_ADRENALINE		, EFST_ADRENALINE		, SCB_ASPD|SCB_HIT );
	set_sc( SK_KN_ADRENALINERUSH		, SC_KNADRENALINE		, EFST_KNADRENALINE		, SCB_ASPD|SCB_HIT );
		

	set_sc( SK_BS_WEAPONSHARPENING	, SC_WEAPONPERFECTION	, EFST_WEAPONPERFECT, SCB_CRI|SCB_WATK );
	set_sc( SK_BS_POWERTHRUST		, SC_OVERTHRUST		, EFST_OVERTHRUST		, SCB_NONE );
	set_sc( SK_BS_MAXIMIZEPOWER		, SC_MAXIMIZEPOWER	, EFST_MAXIMIZE, SCB_REGEN );
	set_sc( SK_AS_CLOAKING		, SC_CLOAKING		, EFST_CLOAKING		, SCB_SPEED );
	set_sc( SK_AS_ENCHANTPOISON	, SC_ENCPOISON		, EFST_ENCHANTPOISON, SCB_ATK_ELE );
	set_sc( SK_TF_POISONREACT		, SC_POISONREACT	, EFST_POISONREACT	, SCB_NONE );
	set_sc( SK_AS_VENOMSPLASHER		, SC_SPLASHER	, EFST_SPLASHER	, SCB_NONE );
	set_sc( SK_NV_TRICKDEAD		, SC_TRICKDEAD		, EFST_TRICKDEAD		, SCB_REGEN );
	set_sc( SK_SM_AUTOBERSERK		, SC_AUTOBERSERK	, EFST_AUTOBERSERK	, SCB_NONE );
	set_sc( SK_SM_SPEARSTANCE			, SC_SPEARSTANCE		, EFST_SPEARSTANCE, SCB_STR|SCB_VIT );
	set_sc( SK_AM_HATCHHOMUNCULUS			, SC_BIOETHICS		, EFST_BIOETHICS, SCB_NONE );
	set_sc( SK_MC_LOUD			, SC_LOUD		, EFST_SHOUT, SCB_STR );
	set_sc( SK_MC_UPROAR			, SC_UPROAR		, EFST_UPROAR, SCB_WATK );
	set_sc( SK_SM_SPIRITGROWTH			, SC_SPIRITGROWTH		, EFST_SPIRITGROWTH, SCB_STR|SCB_INT );
	set_sc( SK_BS_AXEQUICKEN			, SC_AXEQUICKEN		, EFST_AXEQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_RG_DAGGERQUICKEN			, SC_DAGGERQUICKEN		, EFST_DAGGERQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_KN_OHQUICKEN			, SC_OHQUICKEN		, EFST_OHQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_EX_DANCINGBLADES			, SC_DANCINGBLADE		, EFST_DANCINGBLADES, SCB_WATK );
	set_sc( SK_CR_OHQUICKEN			, SC_OHQUICKEN		, EFST_OHQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_KN_SQUICKEN			, SC_SQUICKEN		, EFST_SQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_KN_THQUICKEN			, SC_THQUICKEN		, EFST_THQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_AL_MACEQUICKEN			, SC_MACEQUICKEN		, EFST_MACEQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_MO_KNUCKLEQUICKEN			, SC_KNUCKLEQUICKEN		, EFST_KNUCKLEQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_HT_BOWQUICKEN			, SC_BOWQUICKEN		, EFST_BOWQUICKEN, SCB_ASPD|SCB_WATK );
	set_sc( SK_SA_BOOKQUICKEN			, SC_QUICKSTUDY		, EFST_QUICKSTUDY, SCB_ASPD|SCB_WATK );
	set_sc( SK_HT_CYCLONICCHARGE			, SC_CYCLONICCHARGE		, EFST_CYCLONICCHARGE, SCB_FLEE|SCB_WATK );
	set_sc( SK_MG_ENERGYCOAT		, SC_ENERGYCOAT		, EFST_ENERGYCOAT		, SCB_MATK );
	add_sc( SK_RG_BACKSTAB		, SC_STUN		);

	set_sc( SK_ST_STRIPWEAPON		, SC_STRIPWEAPON	, EFST_NOEQUIPWEAPON, SCB_WATK );
	set_sc( SK_ST_STRIPSHIELD		, SC_STRIPSHIELD	, EFST_NOEQUIPSHIELD, SCB_DEF );
	set_sc( SK_ST_STRIPARMOR		, SC_STRIPARMOR		, EFST_NOEQUIPARMOR, SCB_NONE );
	set_sc( SK_ST_STRIPHELM		, SC_STRIPHELM		, EFST_NOEQUIPHELM, SCB_NONE );
	set_sc( SK_CR_CP_WEAPON		, SC_CP_WEAPON		, EFST_PROTECTWEAPON, SCB_NONE );
	set_sc( SK_CR_CP_SHIELD		, SC_CP_SHIELD		, EFST_PROTECTSHIELD, SCB_NONE );
	set_sc( SK_CR_CP_ARMOR		, SC_CP_ARMOR		, EFST_PROTECTARMOR, SCB_NONE );
	set_sc( SK_CR_CP_HELM		, SC_CP_HELM		, EFST_PROTECTHELM, SCB_NONE );
	set_sc( SK_CR_AUTOGUARD		, SC_AUTOGUARD		, EFST_AUTOGUARD		, SCB_NONE );
	add_sc( SK_CR_SMITE		, SC_STUN		);
	set_sc( SK_CR_REFLECTSHIELD	, SC_REFLECTSHIELD	, EFST_REFLECTSHIELD	, SCB_NONE );
	add_sc( SK_CR_HOLYCROSS		, SC_BLIND		);
	set_sc( SK_CR_SWORNPROTECTOR		, SC_DEVOTION	, EFST_DEVOTION	, SCB_NONE);
	set_sc( SK_PA_DEFENDINGAURA		, SC_DEFENDER		, EFST_DEFENDER		, SCB_SPEED|SCB_ASPD );
	set_sc( SK_SH_MENTALSTRENGTH		, SC_STEELBODY		, EFST_STEELBODY		, SCB_DEF|SCB_MDEF|SCB_ASPD|SCB_SPEED );
	add_sc( SK_SH_GRAPPLE		, SC_BLADESTOP_WAIT	);
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO1	, EFST_COMBO1	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO2	, EFST_COMBO2	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO3	, EFST_COMBO3	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO4	, EFST_COMBO4	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO5	, EFST_COMBO5	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO6	, EFST_COMBO6	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO7	, EFST_COMBO7	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO8	, EFST_COMBO8	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO9	, EFST_COMBO9	, SCB_NONE );
	set_sc( SK_MO_TRIPLEARMCANNON		, SC_COMBO10	, EFST_COMBO10	, SCB_NONE );
	
	set_sc( SK_SH_ULTRAINSTINCT	, SC_EXPLOSIONSPIRITS	, EFST_EXPLOSIONSPIRITS	, SCB_STR|SCB_AGI|SCB_VIT|SCB_WATK );
	set_sc( SK_KN_FURY	, SC_FURY	, EFST_FURY	, SCB_CRI|SCB_WATK );
	set_sc( SK_BI_LAUDATEDOMINIUM	, SC_LAUDATEDOMINIUM	, EFST_SK_BI_LAUDATEDOMINIUM	,  SCB_MATK );
	set_sc( SK_SA_AUTOSPELL		, SC_AUTOSPELL		, EFST_AUTOSPELL		, SCB_NONE );
	
	set_sc( SK_BA_ACOUSTICRYTHM		, SC_RICHMANKIM		, EFST_RICHMANKIM	, SCB_STR|SCB_VIT|SCB_MAXHP|SCB_MAXSP );
	set_sc( SK_CL_BATTLEDRUMS	, SC_DRUMBATTLE		, EFST_DRUMBATTLEFIELD	, SCB_WATK|SCB_DEF|SCB_MATK|SCB_MDEF );

	set_sc( SK_BA_MOONLIGHTSONATA		, SC_INTOABYSS	, EFST_INTOABYSS	, SCB_NONE );
	set_sc( SK_CL_PAGANPARTY		, SC_SIEGFRIED		, EFST_SIEGFRIED	, SCB_NONE );
	set_sc( SK_BA_IMPRESSIVERIFF	, SC_ASSNCROS		, EFST_ASSASSINCROSS		, SCB_AGI|SCB_LUK|SCB_ASPD|SCB_CRI );
	set_sc( SK_BA_MAGICSTRINGS		, SC_POEMBRAGI	, EFST_POEMBRAGI	, SCB_INT|SCB_DEX	);
	set_sc( SK_CM_AURABLADE		, SC_AURABLADE		, EFST_AURABLADE		, SCB_NONE );
	set_sc( SK_CM_PARRY		, SC_PARRYING		, EFST_PARRYING		, SCB_NONE );
	set_sc( SK_CM_SPEAR_DYNAMO	, SC_CONCENTRATION	, EFST_LKCONCENTRATION	, SCB_HIT );
	set_sc( SK_PA_FORTIFY	, SC_FORTIFY	, EFST_FORTIFY	, SCB_DEF|SCB_MDEF );
	set_sc( SK_CM_FRENZY		, SC_BERSERK		, EFST_BERSERK		, SCB_SPEED|SCB_ASPD );
	set_sc( SK_BI_ASSUMPTIO		, SC_ASSUMPTIO		,EFST_ASSUMPTIO		, SCB_NONE );

	set_sc( SK_SO_MYSTICALAMPLIFICATION		, SC_MAGICPOWER		, EFST_MAGICPOWER		, SCB_MATK );
	set_sc( SK_KN_RECKONING		, SC_SACRIFICE		, EFST_SACRIFICE, SCB_NONE		);
	set_sc( SK_HT_LIVINGTORNADO		, SC_HURRICANEFURY		, EFST_HURRICANEFURY, SCB_NONE		);
	set_sc( SK_RA_ZEPHYRSNIPING		, SC_ZEPHYR_SNIPING		, EFST_ZEPHYR_SNIPING, SCB_NONE		);
	set_sc( SK_PA_GOSPEL		, SC_GOSPEL		, EFST_GOSPEL		, SCB_SPEED|SCB_ASPD );
	add_sc( SK_PA_GOSPEL		, SC_SCRESIST		);
	set_sc( SK_EX_ENCHANTDEADLYPOISON			, SC_EDP		, EFST_EDP		, SCB_NONE );
	set_sc( SK_RA_FALCONEYES		, SC_TRUESIGHT		, EFST_TRUESIGHT		, SCB_STR|SCB_AGI|SCB_VIT|SCB_INT|SCB_DEX|SCB_LUK|SCB_CRI|SCB_HIT );
	set_sc( SK_HT_WINDRACER		, SC_WINDWALK		, EFST_WINDWALK		, SCB_AGI|SCB_SPEED );
	set_sc( SK_MS_MELTDOWN		, SC_MELTDOWN		, EFST_MELTDOWN		, SCB_WATK );
	set_sc( SK_BS_CARTBOOST		, SC_CARTBOOST		, EFST_CARTBOOST		, SCB_SPEED|SCB_WATK );
	set_sc( SK_AS_STEALTH		, SC_CHASEWALK		, EFSK_AS_STEALTH		, SCB_SPEED );
	set_sc( SK_ST_KILLERINSTIINCT		, SC_REJECTSWORD	, EFST_SWORDREJECT, SCB_WATK|SCB_CRI );
	set_sc( SK_CL_MARIONETTECONTROL		, SC_MARIONETTE		, EFST_MARIONETTE_MASTER, SCB_STR|SCB_AGI|SCB_VIT|SCB_INT|SCB_DEX|SCB_LUK );
	set_sc( SK_CL_MARIONETTECONTROL		, SC_MARIONETTE2	, EFST_MARIONETTE, SCB_STR|SCB_AGI|SCB_VIT|SCB_INT|SCB_DEX|SCB_LUK );
	add_sc( SK_SM_TRAUMATIC_STRIKE		, SC_BLEEDING		);
	set_sc( SK_SA_MINDGAMES		, SC_MINDBREAKER	, EFST_MINDBREAKER	, SCB_MATK|SCB_MDEF2 );
	set_sc( SK_WZ_FORESIGHT		, SC_MEMORIZE	, EFST_MEMORIZE	, SCB_NONE );
	set_sc( SK_WZ_ILLUSIONARYBLADES			, SC_SMA		, EFST_SMA_READY, SCB_NONE );
	set_sc( SK_PF_MAGICSQUARED	, SC_DOUBLECAST		, EFST_DOUBLECASTING, SCB_NONE );

	set_sc( SK_MS_MAXPOWERTHRUST	, SC_MAXOVERTHRUST	, EFST_OVERTHRUSTMAX, SCB_NONE );


	set_sc( SK_RG_CLOSECONFINE		, SC_CLOSECONFINE2	, EFST_RG_CCONFINE_S, SCB_NONE );
	set_sc( SK_RG_CLOSECONFINE		, SC_CLOSECONFINE	, EFST_RG_CCONFINE_M, SCB_FLEE );
	add_sc( SK_MO_FALINGFIST		, SC_STUN		);


	
	/* Rune Knight */
	set_sc( SK_KN_ENCHANTBLADE		, SC_ENCHANTBLADE	, EFST_ENCHANTBLADE		, SCB_NONE );
	set_sc( SK_KN_STONESKIN	, SC_STONEHARDSKIN	, EFST_STONEHARDSKIN		, SCB_DEF|SCB_MDEF );
	set_sc( SK_MO_BODYRELOCATION	, SC_DEFENSIVESTANCE	, EFST_DEFENSIVESTANCE		, SCB_DEF|SCB_MDEF );
	set_sc( SK_KN_VIGOR	, SC_VITALITYACTIVATION	, EFST_VITALITYACTIVATION		, SCB_VIT );
	
	/* GC Guillotine Cross */
	set_sc_with_vfx( SK_AS_VENOMIMPRESS, SC_VENOMIMPRESS	, EFST_VENOMIMPRESS	, SCB_NONE );
	set_sc( SK_EX_HALLUCINATIONWALK	, SC_HALLUCINATIONWALK	, EFST_HALLUCINATIONWALK	, SCB_FLEE );
	set_sc( SK_AS_ROLLINGCUTTER	, SC_ROLLINGCUTTER	, EFST_ROLLINGCUTTER	, SCB_NONE );
	set_sc_with_vfx( SK_EX_DARKCLAW	, SC_DARKCROW		, EFST_DARKCROW		, SCB_NONE );

	/* Arch Bishop */
	set_sc( SK_BI_EPICLESIS		, SC_EPICLESIS		, EFST_EPICLESIS, SCB_NONE);
	set_sc( SK_AL_RENEW		, SC_RENOVATIO		, EFST_RENOVATIO		, SCB_REGEN );
	set_sc( SK_BI_EXPIATIO		, SC_EXPIATIO		, EFST_EXPIATIO		, SCB_NONE );
	set_sc( SK_BI_INLUCEMBATALIA		, SC_ULTOR		, EFST_ULTOR		, SCB_WATK|SCB_MATK|SCB_ASPD );
	set_sc( SK_PR_DUPLELUX		, SC_DUPLELIGHT		, EFST_DUPLELIGHT		, SCB_NONE );
	set_sc( SK_BI_OFFERTORIUM		, SC_OFFERTORIUM	, EFST_OFFERTORIUM	, SCB_NONE );
	add_sc( SK_PR_LEXAETERNA		, SC_AETERNA );

	/* Warlock */
	add_sc( SK_SA_WHITEIMPRISON	, SC_WHITEIMPRISON	);
	set_sc( SK_SO_HINDSIGHT	, SC_RECOGNIZEDSPELL	, EFST_RECOGNIZEDSPELL	, SCB_MATK);
	set_sc( SK_SO_VOIDMAGEPRODIGY	, SC_TELEKINESIS_INTENSE, EFST_TELEKINESIS_INTENSE, SCB_MATK );
	set_sc( SK_CR_GEOGRAFIELD_ATK	, SC_GEOGRAFIELD, EFST_GEOGRAFIELD, SCB_NONE );

	/* Ranger */
	set_sc( SK_RA_TYHPOONFLOW		, SC_FEARBREEZE		, EFST_FEARBREEZE		, SCB_NONE );
	set_sc( SK_HT_CAMOUFLAGE		, SC_CAMOUFLAGE		, EFST_CAMOUFLAGE		, SCB_SPEED );
	set_sc( SK_ST_UNLIMIT		, SC_UNLIMIT		, EFST_UNLIMIT		, SCB_NONE);

	/* Mechanic */
	set_sc( SK_MS_NEUTRALBARRIER	, SC_NEUTRALBARRIER	, EFST_NEUTRALBARRIER	, SCB_DEF|SCB_MDEF );

	/* Royal Guard */
	set_sc( SK_CR_VANGUARDFORCE		, SC_PRESTIGE		, EFST_PRESTIGE		, SCB_DEF );
	set_sc( SK_CM_MILLENIUMSHIELDS		, SC_KINGS_GRACE	, EFST_KINGS_GRACE	, SCB_NONE );

	/* Shadow Chaser */
	set_sc( SK_ST_REPRODUCE		, SC__REPRODUCE		, EFST_REPRODUCE		, SCB_NONE );
	set_sc( SK_RG_PLAGIARISM		, SC_PLAGIARISM		, EFST_PLAGIARISM		, SCB_NONE );
	set_sc( SK_ST_AUTOSHADOWSPELL	, SC__AUTOSHADOWSPELL	, EFST_AUTOSHADOWSPELL	, SCB_NONE );
	set_sc( SK_ST_STALK		, SC__SHADOWFORM	, EFST_SHADOWFORM		, SCB_NONE );
	set_sc( SK_EX_INVISIBILITY		, SC__INVISIBILITY	, EFST_INVISIBILITY	, SCB_NONE );
	set_sc( SK_RG_STRIPACCESSORY	, SC__STRIPACCESSORY	, EFST_STRIPACCESSARY	, SCB_DEX|SCB_INT|SCB_LUK|SCB_STR|SCB_AGI|SCB_VIT );

	/* Sura */

	/* Wanderer / Minstrel */
	set_sc( SK_BA_WINDMILLPOEM		, SC_RUSHWINDMILL		, EFST_RUSH_WINDMILL, SCB_WATK|SCB_DEF );
	set_sc( SK_BA_ECHONOCTURNAE			, SC_ECHOSONG			, EFST_ECHOSONG			, SCB_MATK|SCB_DEF  );
	set_sc_with_vfx( SK_CL_DEEPSLEEPLULLABY	, SC_SLEEP			, EFST_SLEEP, SCB_NONE );
	set_sc( SK_PF_STASIS	, SC_STASIS			, EFST_STASIS, SCB_NONE );

	/* Sorcerer */
	set_sc( SK_PF_SPELLFIST		, SC_SPELLFIST		, EFST_SPELLFIST		, SCB_NONE );
	set_sc( SK_AS_POISONOUSCLOUD   , SC_POISON         , EFST_CLOUD_KILL, SCB_NONE );

	/* Genetic */
	set_sc_with_vfx( SK_RA_ULLREAGLETOTEM		, SC_ULLR	, EFST_ULLR	, SCB_NONE );
	...
	StatusIconChangeTable[SC_CHASEWALK2] = EFSK_AS_STEALTH2;
	...
	StatusChangeFlagTable[SC_CHASEWALK2] |= SCB_STR|SCB_DEX;
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag) {
	...
	if( sc && sc->count ) {
		...
		if (
			(sc->data[SC_TRICKDEAD] && skill_id != SK_NV_TRICKDEAD)
			|| (sc->data[SC_AUTOCOUNTER] && !flag && skill_id)
			// || (sc->data[SC_GOSPEL] && sc->data[SC_GOSPEL]->val4 == BCT_SELF && skill_id != SK_PA_GOSPEL
		)
			return false;
		...
		if (skill_id && // Do not block item-casted skills.
...
#endif
				// (sc->data[SC_MARIONETTE] && skill_id != SK_CL_MARIONETTECONTROL) || // Only skill you can use is marionette again to cancel it
				(sc->data[SC_MARIONETTE2] && skill_id == SK_CL_MARIONETTECONTROL) || // Cannot use marionette if you are being buffed by another
			...
			// Skill blocking.
			if (
				(sc->data[SC_VOLCANO] && skill_id == SK_MG_ICEWALL) ||
	...
	if (tsc && tsc->count) {
		...
		**/
		if( tsc->data[SC_INVINCIBLE] && !tsc->data[SC_INVINCIBLEOFF] && skill_id )
			return false;
		...
		if((skill_id == SK_WZ_STORMGUST)
			&& tsc->data[SC_FREEZE])
			return false;
		if(skill_id == SK_PR_LEXAETERNA && (tsc->data[SC_FREEZE] || (tsc->data[SC_STONE] && tsc->opt1 == OPT1_STONE)))
			return false;
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	if (sc->data[SC_DANCING] ||
		...
		/*(bl->type == BL_PC && (((TBL_PC*)bl)->class_&MAPID_UPPERMASK) == MAPID_MONK && (!sc->data[SC_SPIRIT] || sc->data[SC_SPIRIT]->val2 != SL_MONK)) ||*/
...
static unsigned short status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	...
	if(sc->data[SC_INCREASEAGI])
		// agi +=(sc->data[SC_INCREASEAGI]->val1/agi)*100;
	...
	if(sc->data[SC_DECREASEAGI])
		agi -=(sc->data[SC_DECREASEAGI]->val1/agi)*100;
...
static unsigned short status_calc_watk(struct block_list *bl, struct status_change *sc, int watk)
{
	...
	if (sc->data[SC_KNUCKLEQUICKEN])
		watk += 3*sc->data[SC_KNUCKLEQUICKEN]->val1;
...
static signed short status_calc_flee(struct block_list *bl, struct status_change *sc, int flee)
{
	...
	if(sc->data[SC_CLOSECONFINE])
		flee += sc->data[SC_CLOSECONFINE]->val1*10;
	if (sc->data[SC_ANGRIFFS_MODUS])
...
static short status_calc_aspd(struct block_list *bl, struct status_change *sc, bool fixed)
{
	...
	if (fixed) {
		...
		if (sc->data[SC_WIND_INSIGNIA] && sc->data[SC_WIND_INSIGNIA]->val1 == 2)
			bonus += 10;
		if (sc->data[SC_INCREASEAGI])
			bonus += sc->data[SC_INCREASEAGI]->val1 * 2;
...
//		if (util::vector_exists(status_get_race2(bl), RC2_GVG) && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_APPLEIDUN:
		{
			...
			val2 = (5 + 2 * val1) + (status_get_vit(src) / 10); //HP Rate: (5 + 2 * skill_lv) + (vit/10) + (SK_BA_SHOWMANSHIP level)
			if (s_sd)
				val2 += pc_checkskill(s_sd, SK_BA_SHOWMANSHIP) / 2;
			break;
		}
#endif
...
int status_change_timer_sub(struct block_list* bl, va_list ap)
{
	...
	case SC_RUWACH: // Reveal hidden target and deal little dammages if enemy
		if (tsc && (tsc->data[SC_HIDING] || tsc->data[SC_CLOAKING] ||
				tsc->data[SC_CAMOUFLAGE] || tsc->data[SC__INVISIBILITY] || tsc->data[SC__SHADOWFORM] )) {
			status_change_end(bl, SC_HIDING, INVALID_TIMER);
			status_change_end(bl, SC_CLOAKING, INVALID_TIMER);
			status_change_end(bl, SC_CAMOUFLAGE, INVALID_TIMER);
			status_change_end(bl, SC_NEWMOON, INVALID_TIMER);
