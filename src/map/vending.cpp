...
int8 vending_openvending(struct map_session_data* sd, const char* message, const uint8* data, int count, struct s_autotrader *at)
{
	...
	vending_skill_lvl = pc_checkskill(sd, SK_MC_VENDING);
	
	// skill level and cart check
	if( !vending_skill_lvl || !pc_iscarton(sd) ) {
		clif_skill_fail(sd, SK_MC_VENDING, USESKILL_FAIL_LEVEL, 0);
		return 2;
	}
	...
	if (i != j) {
		...
		clif_skill_fail(sd, SK_MC_VENDING, USESKILL_FAIL_LEVEL, 0); // custom reply packet
		return 5;
	}
